...
model = YOLO(r"C:\Users\rames\Downloads\weights-20241104T051500Z-001\weights\best.pt")  # Replace with your custom helmet model if available
...
while cap.isOpened():
    ...
    for box in results[0].boxes:
        ...
        if class_id == helmet_class_id:  # Check if the detected class is a helmet
            ...
            # Draw the same bounding box on the original frame for saving
            cv2.rectangle(frame, (x1, head_y1), (x2, head_y2), (0, 255, 0), 2)  # Draw on the frame to save
            cv2.putText(frame, "Helmet", (x1, head_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    # Add text based on detection
    if helmet_detected:
        cv2.putText(img, "Helmet Detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)  # Green text
        # Save the frame with the bounding box if a helmet is detected
...
results = model(frame, conf=0.25)  # Adjust conf as needed
